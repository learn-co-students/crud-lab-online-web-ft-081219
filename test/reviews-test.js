...
describe("ReviewInput Component", () => {
  ...
  it("adds a review to the store when the form is submitted", () => {
    ...
    input.simulate("change", {
      target: { value: "Better Ingredients", name: "text", id: "text" }
    });
  ...
  it("updates the state of the store after submitting the form", () => {
    ...
    input.simulate("change", {
      target: { value: "Better Pizza", name: "text", id: "text" }
    });
    form.simulate("submit", { preventDefault() {} });
  ...
  it("sets a property of restaurantId on the review input component from the parent components id", () => {
    ...
    expect(review.props().restaurantId).to.equal(
      store.getState().restaurants[0].id
    );
  });
  ...
  it("associates the review with the restaurant with a foreign key on the review", () => {
    ...
    expect(store.getState().reviews[0].restaurantId).to.equal(
      store.getState().restaurants[0].id
    );
  });
});
...
describe("Reviews Component", () => {
  it("is a child of the ReviewsContainer component", () => {
    ...
    expect(wrapper.find(ReviewsContainer).html()).to.include(
      wrapper.find(Reviews).html()
    );
  });
  ...
  it("displays a review for when it is associated with the restaurant", () => {
    ...
    store.dispatch({
      type: "ADD_REVIEW",
      review: { text: "Was great", restaurantId }
    });
    store.dispatch({
      type: "ADD_REVIEW",
      review: { text: "Was not great", restaurantId }
    });
    const wrapper = mount(
      <Provider store={store}>
        <App />
  ...
  it("does not display any review unassociated with the restaurant", () => {
    ...
    store.dispatch({
      type: "ADD_REVIEW",
      review: { text: "it was good", restaurantId }
    });
    store.dispatch({
      type: "ADD_REVIEW",
      review: { text: "it was great", restaurantId }
    });
    store.dispatch({
      type: "ADD_REVIEW",
      review: { text: "it was bad", restaurantId: "test" }
    });
    const wrapper = mount(
      <Provider store={store}>
        <App />
  ...
  it("has an unique id property that for each element", () => {
    ...
    textField.simulate("change", {
      target: {
        value: "Walked for miles at night for this place",
        name: "text",
        id: "text"
      }
    });
    ...
    textField.simulate("change", {
      target: { value: "Totally worth it", name: "text", id: "text" }
    });
    reviewForm.simulate("submit", { preventDefault() {} });
  ...
  it("has a button that dispatches a DELETE_REVIEW action when clicked", () => {
    ...
    store.dispatch({
      type: "ADD_REVIEW",
      review: { text: "became friends with bartender", restaurantId }
    });
  ...
  it("updates the state of the store to remove the component", () => {
    ...
    input.simulate("change", {
      target: { value: "Pricey", name: "text", id: "text" }
    });
    form.simulate("submit", { preventDefault() {} });
    input.simulate("change", {
      target: {
        value: "I would rather walk 4 miles at night to the next town",
        name: "text",
        id: "text"
      }
    });
